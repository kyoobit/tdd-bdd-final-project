...
class TestProductRoutes(TestCase):
    ...
    def test_get_product_not_found(self):
        ...
        # product ID
        response = self.client.get(f"{BASE_URL}/99999999")
        # assert that the resp.status_code is status.HTTP_404_NOT_FOUND
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        # get the data from resp.get_json()
        data = response.get_json()
        # assert that data["message"] contains "was not found"
    ...
    def test_query_by_availability(self):
        ...
        logging.debug("Available Products: %d", available_count)
        # Send an HTTP GET request to the URL specified by the BASE_URL
        # variable, along with a query parameter "available" set to true.
    ...
    def test_query_by_category(self):
        ...
        products = self._create_products(10)
        # retrieves the category of the first product in the products list and
        # assigns it to the variable category
        ...
        # Log a debug message indicating the count and details of the
        # products found
        logging.debug("Found Products [%d] %s", found_count, found)
        # Send an HTTP GET request to the URL specified by the BASE_URL
        # variable, along with a query parameter "category"
    ...
    def test_query_by_name(self):
        ...
        # count the number of products in the products list that have the same
        # name as the test_name
        name_count = len([product for product in products if product.name == test_name])
        # send an HTTP GET request to the URL specified by the BASE_URL
        # variable, along with a query parameter "name"
        response = self.client.get(
        ...
        # use a for loop to iterate through the products in the data list and
        # checks if each product's name matches the test_name
        for product in data:
    ...
    def test_update_product_not_found(self):
        """It should not Update a non-exist Product"""
        product_id = 99999999
        # Assert a GET for the product ID returns a 404 File Not Found
        response = self.client.get(f"{BASE_URL}/{product_id}")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        # Send a self.client.put() request to the BASE_URL with a json payload
        response = self.client.put(f"{BASE_URL}/{product_id}", json={"test": True})
        # Assert that the resp.status_code is status.HTTP_404_NOT_FOUND
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
